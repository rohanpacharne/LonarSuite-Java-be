...
//			status=ltMastCommonMessageService.getCodeAndMessage(INPUT_IS_EMPTY);
		...
		ltVendCompanyMiscellaneous = ltVendCompanyMiscellaneousRepository.save(ltVendCompanyMiscellaneous);
			...
			{
				updateCompanyVenMisc(ltVendCompanyMiscellaneous.getCompanyId());
...
//			status=ltMastCommonMessageService.getCodeAndMessage(INPUT_IS_EMPTY);
	...
	private void updateCompanyVenMisc(Long companyId) throws ServiceException 
	{
		LtVendCompanyMiscellaneousThread vendCompanyMiscellaneousThread =
				new LtVendCompanyMiscellaneousThread(ltVendCompanyMiscellaneousDao,ltVendCompanyDao,
						ltCompanyVenMgmtMiscRepository,ltCompanyVenMgmtMiscQuesRepository,ltVendMiscQuestionsDao,companyId);
		Thread t1 =new Thread(vendCompanyMiscellaneousThread);  
		t1.start();
	}
	...
	public ResponseEntity<Status> delete(Long id) throws ServiceException {
		Status status = new Status();
		LtVendCompanyMiscellaneous ltVendCompanyMiscellaneous = ltVendCompanyMiscellaneousRepository.findOne(id);
