...
//	}
	...
	@Override
    public Status createPoPdfReport(Long poHeaderId,Long companyId) throws ServiceException {
        Status status = new Status();
...
//            String pdfOutputPath = "D:/Lexa/PO_PDF_TEMP/POReport_" + poHeaderId + ".pdf";
    ...
    @Override
	public Status submitForApproval(Date date, Long poHeaderId, String statusStr, Object object,Long userId, Long companyId)
			throws ServiceException {
		Status status = new Status();
		LtPoHeaders poHeaders = ltPoHeadersDao.getPoHeaderById(poHeaderId);
		System.out.println("invoiceHeaders = "+poHeaders);
		status = checkPoMandatory(poHeaders);
		System.out.println("in submit for approval = "+status);
		if(status.getCode() !=null) {
			return status;
		}
		
		if(poHeaders.getStatus().equals("PO_ACTIVE"))
		{
			ltPoHeadersDao.upDateStatus(poHeaderId, NO_ACTION, null);
		}
		if(poHeaders.getPoAmount()== null || poHeaders.getPoAmount()<=0) {
			status.setCode(0);
			status.setMessage("Po with 0 amount can not be sent for approval.");
		}
		
		
		if(!ltPoHeadersDao.chkForApprovers(poHeaderId)) {
			System.out.println(!ltPoHeadersDao.chkForApprovers(poHeaderId));
		if(!ltMastVendorsDao.loadPoApprovers(poHeaders)){
		System.out.println(!ltMastVendorsDao.loadPoApprovers(poHeaders));
//			status=ltMastCommonMessageService.getCodeAndMessage(INTERNAL_SERVER_ERROR);
		...
		//----------------------------call proc here --------------------
//		status = ltPoHeadersDao.callPoValidationProc(poHeaderId);
//		System.out.println("prcedure status code = "+status.getCode());
//		if(status.getCode().equals(1)) {
			if(ltPoHeadersDao.submitForApproval(date,poHeaderId,statusStr,object)) {
...
//				if(updatePoShipmentQuantitiesProcstatus.getCode().equals(1)) {
				try {
					status.setCode(1);
					status.setMessage(ltMastCommonMessageService.getMessageNameByCode("PO_SUBMIT_FOR_APPROVAL").getMessageName());
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
...
//		}
    ...
    private Status checkPoMandatory(LtPoHeaders poHeaders) {
		Status status = new Status();
		if(poHeaders.getPoNumber()==null) {
			status.setCode(0);
			status.setMessage("Please insert Invoice Number");
		}else if(poHeaders.getPoDate() == null) {
			status.setCode(0);
			status.setMessage("Please insert Invoice Date");
		}else if(poHeaders.getBuyerId() == null) {
			status.setCode(0);
			status.setMessage("Please select Buyer");
		}else if(poHeaders.getVendorAddId() == null) {
			status.setCode(0);
			status.setMessage("Please select Vendor Address");
		}else if(poHeaders.getBillingAddId() == null) {
			status.setCode(0);
			status.setMessage("Please select Billing Address");
		}else if(poHeaders.getDescription() == null) {
			status.setCode(0);
	...
	public String checkforApprovals(Long poHeaderId) throws ServiceException {
		return ltPoHeadersDao.checkforApprovals(poHeaderId);
	}
    
    @Override
	public boolean checkStatusIsPending(Long poHeaderId, Long approvalId) throws ServiceException {
		return ltPoHeadersDao.checkStatusIsPending(poHeaderId, approvalId);
	}
    
    @Override
	public LtPoHeaders getPoStatusById(Long poHeaderId) throws ServiceException {
		return ltPoHeadersDao.getPoStatusById(poHeaderId);
	}
    
    @Override
	public Status save(LtPoHeaders ltPoHeaders) throws ServiceException{
		Status status=new Status();
		String chknull=checkNull(ltPoHeaders);
...
//			status=ltMastCommonMessageService.getCodeAndMessage(INPUT_IS_EMPTY);
		...
		{
			String chkDuplicate= checkDuplicate(ltPoHeaders,"save");
			if(chkDuplicate.equals("null"))
			{
				ltPoHeaders.setCreatedBy(ltPoHeaders.getLastUpdateLogin());
				ltPoHeaders.setLastUpdatedBy(ltPoHeaders.getLastUpdateLogin());
				ltPoHeaders.setCreationDate(new Date());
				ltPoHeaders.setLastUpdateDate(new Date());
				Long poHeaderId=ltPoHeadersDao.save(ltPoHeaders);
				if(poHeaderId!=null)
				{
					//ltInvoiceHeaders.setInternalInvoiceNumber("IN||"+invoiceHeaderId);
					ltPoHeaders.setPoHeaderId(poHeaderId);
					ltPoHeadersDao.save(ltPoHeaders);   
					...
					boolean result= !ltPoHeadersDao.loadApprovers(ltPoHeaders);
					System.out.println("return loadApproval::"+result);
					if(result ){
...
//					status=ltMastCommonMessageService.getCodeAndMessage(INSERT_FAIL);
	...
	private String checkDuplicate(LtPoHeaders ltPoHeaders, String flag) throws ServiceException
	{
		LtPoHeaders PoHeaders = ltPoHeadersDao.getByPoNumVendAndAddr(ltPoHeaders.getPoNumber(),ltPoHeaders.getVendorId(),
				ltPoHeaders.getVendorAddId());
		if(PoHeaders!=null) {
			if(!PoHeaders.getPoHeaderId().equals(ltPoHeaders.getPoHeaderId())
					&& !PoHeaders.getPoHeaderId().equals(ltPoHeaders.getPoHeaderId())) {
	...
	private String checkNull(LtPoHeaders ltPoHeaders)
	{
		if(ltPoHeaders.getCreatedBy()==null || ltPoHeaders.getCreationDate()==null ||
				ltPoHeaders.getLastUpdateLogin()==null)
		{
			return "null";
		}
		else
	...
	public Status delete(Long id) {
		Status status = new Status();
		ltPoHeadersRepository.delete(id);
		if(ltPoHeadersRepository.exists(id)) {
