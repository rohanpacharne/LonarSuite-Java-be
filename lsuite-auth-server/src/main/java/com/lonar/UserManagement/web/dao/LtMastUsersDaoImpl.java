...
//				"                 AND vm.INITIATOR_ID = em.EMPLOYEE_ID(+)  AND vm.DIVISION_ID = d.DIVISION_ID(+)  ";
	...
	public List<Menu> findReport(Long userId, Long companyId) {
		// TODO Auto-generated method stub
		String sqlQuery = env.getProperty("findReportMenu");
		   ...
		   return jdbcTemplate.query(sqlQuery,new Object[]{userId,companyId}, new RowMapper<Menu>(){
			...
			public Menu mapRow(ResultSet rs, int row)
				...
				System.out.println(rs.getFetchSize());
					...
					menu.setDelete(rs.getString("DELETE_FLAG"));
					menu.setUpdate(rs.getString("UPDATE_FLAG"));
					menu.setModuleDesc(rs.getString("MODULE_DESC"));
					return menu;
				}
	...
	@Override
	public String mfaCheck(LtMastUsers ltMastUsers) {
		
		String sql = "select mfa from lt_mast_users lmu where upper(USER_NAME) = ? and PASSWORD = ?";
        // Call the function and get the result as an Integer
        return jdbcTemplate.queryForObject(sql, new Object[]{ltMastUsers.getUserName(), ltMastUsers.getPassword()}, String.class);
	}
